...
import { newContractItemValidate } from '@/types/Contract/tContractItem';
...
export async function GET(
  ...
  const registryList = await prisma.contractItem.findMany({
    where: {
...
export async function POST(
  ...
  const validatedSchema = newContractItemValidate.safeParse(body);
  ...
  try {
    newRegistry = await prisma.contractItem.create({
      data: {
        contractId: params.id,

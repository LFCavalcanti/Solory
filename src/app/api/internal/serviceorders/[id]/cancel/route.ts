...
export async function POST(
  ...
  try {
    serviceOrder = await prisma.serviceOrder.update({
      ...
      data: {
        isActive: false,
        disabledAt: currDate.toISOString(),
      },
    });
    await prisma.serviceOrderEvent.create({
      data: {
        ...
        description: `ORDEM DE SERVIÇO CANCELADA POR ${session.user.name} EM ${currDate.toLocaleDateString()}`,
        userId: session.user.id,
      },
    });
  } catch (error) {
    ...
    return new NextResponse(
      JSON.stringify({
        message: 'Error canelling the Service Order',
      }),
      {
        status: 500,
      },

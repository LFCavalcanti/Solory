...
interface Props {
  ...
  addressData?: tSupplierAddress | null;
...
export default function SupplierAddressForm({
  ...
  const {
    ...
    resolver: zodResolver(supplierAddressValidate),
  });

  const title = getTitleByAction('EMPRESA - ENDEREÇO', formAction);

  const submitSupplierAddress: SubmitHandler<tSupplierAddress> = async (
    data,
  ) => {
    ...
    if (formAction === 'delete') {
      try {
        updatedData = await fetchApp({
          ...
          endpoint: `/api/internal/suppliers/${supplierData?.id}/addresses/${addressData?.id}`,
    ...
    try {
      ...
      updatedData = await fetchApp({
        ...
        endpoint:
          formAction === 'insert'
            ? `/api/internal/suppliers/${supplierData?.id}/addresses`
            : `/api/internal/suppliers/${supplierData?.id}/addresses/${addressData?.id}`,
  ...
  useEffect(() => {
    ...
    if (!addressData && formAction !== 'insert') {
      stopProcessingSpinner();
      setIsFormOpen(false);
      throw new Error('Must provide Supplier Address data');
    }

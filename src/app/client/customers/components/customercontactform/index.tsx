...
import {
  ...
  tCustomerContact,
} from '@/types/Customer/tCustomerContact';
...
interface Props {
  ...
  contactData?: tCustomerContact | null;
  customerData?: tCustomer | null;
}
...
export default function CustomerContactForm({
  ...
  const [insertContact, updateContact, removeContact] = useCustomerContactStore(
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<tCustomerContact>({
    resolver: zodResolver(customerContactValidate),
  });

  const title = getTitleByAction('CLIENTE - CONTATO', formAction);

  const submitCustomerAddress: SubmitHandler<tCustomerContact> = async (
    data,
  ) => {
    ...
    if (formAction === 'delete') {
      try {
        updatedData = await fetchApp({
          ...
          endpoint: `/api/internal/customers/${customerData?.id}/contacts/${contactData?.id}`,
    ...
    try {
      ...
      updatedData = await fetchApp({
        ...
        endpoint:
          formAction === 'insert'
            ? `/api/internal/customers/${customerData?.id}/contacts`
            : `/api/internal/customers/${customerData?.id}/contacts/${contactData?.id}`,
  ...
  useEffect(() => {
    ...
    if (!contactData && formAction !== 'insert') {
      stopProcessingSpinner();
      setIsFormOpen(false);
      throw new Error('Must provide Customer Contact data');
    }
          ...
          >
            ...
            <form
              onSubmit={
                formAction === 'view'
                  ? () => setIsFormOpen(false)
                  : handleSubmit(submitCustomerAddress)
              }
            >

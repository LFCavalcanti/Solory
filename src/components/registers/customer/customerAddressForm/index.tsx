...
interface Props {
  ...
  addressData?: tCustomerAddress | null;
...
export default function CustomerAddressForm({
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<tCustomerAddress>({
    resolver: zodResolver(customerAddressValidate),
  });

  const title = getTitleByAction('CLIENTE - ENDEREÇO', formAction);

  const submitCustomerAddress: SubmitHandler<tCustomerAddress> = async (
    data,
  ) => {
    ...
    if (formAction === 'delete') {
      try {
        updatedData = await fetchApp({
          ...
          endpoint: `/api/internal/customers/${customerData?.id}/addresses/${addressData?.id}`,
    ...
    try {
      ...
      updatedData = await fetchApp({
        ...
        endpoint:
          formAction === 'insert'
            ? `/api/internal/customers/${customerData?.id}/addresses`
            : `/api/internal/customers/${customerData?.id}/addresses/${addressData?.id}`,
  ...
  useEffect(() => {
    ...
    if (!addressData && formAction !== 'insert') {
      stopProcessingSpinner();
      setIsFormOpen(false);
      throw new Error('Must provide Customer Address data');
    }
